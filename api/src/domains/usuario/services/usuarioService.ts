...
class UsuarioService {
  ...
  async validateToken(email: string, token: string, password: string) {
    ...
    if (
      user?.tokenRecPass != token ||
      (user.dateRecPass != null && timeNow > user.dateRecPass)
    ) {
      throw new InvalidParamError(
        "Token Inválido, verifique seus dados e tente novamente!",
      );
    }
  ...
  async createToken(email: string) {
    ...
    await enviaEmail(
      ...
      "Aqui está o seu código para recuperação de senha! Lembre-se de não compartilhar esse código com ninguém.",
    );
  }
}
