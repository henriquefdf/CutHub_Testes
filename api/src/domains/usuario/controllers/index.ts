...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      await usuarioService.validateToken(
        req.body.email,
        req.body.token,
        req.body.senha,
      );
      res
        .json("Token validado corretamente!")
        .status(codigoStatus.SUCESSO)
        .end();
    } catch (error) {
      next(error);
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const novoUsuario = await usuarioService.criar(
        req.body,
        req.file as unknown as Express.MulterS3.File,
      );
