...
export class servicoService {
  async criarServico(
    body: ServicoInterface,
    donoId: number,
    file: Express.MulterS3.File | null,
  ) {
    const barbearia = await prisma.barbearia.findFirst({
      where: { usuarioId: donoId },
    });

    if (!barbearia) throw new QueryError("Barbearia não encontrada");

    const servicoExistente = await prisma.servico.findFirst({
      where: { nome: body.nome, barbeariaId: barbearia.id },
    });

    if (servicoExistente) throw new QueryError("Serviço já existe.");
  ...
  async editarServico(
    ...
    if (!servico) throw new QueryError("Serviço não encontrado.");
