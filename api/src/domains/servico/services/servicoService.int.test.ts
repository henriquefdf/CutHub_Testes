...
beforeAll(async () => {
  ...
  const user = await prisma.usuario.create({
    data: { nome:'Dono', email:'dono@x.com', senha:'x', tipo:'dono_barbearia' }
  })
  donoId = user.id
  ...
  const user2 = await prisma.usuario.create({
    data: { nome:'Outro', email:'outro@x.com', senha:'x', tipo:'dono_barbearia' }
  })
  ...
  const serv = await prisma.servico.create({
    data:{
      nome:'Corte', descricao:'desc', preco:50, barbeariaId: barb.id, foto: 'init.jpg',
      chaveAws: 'init.jpg',
    }
  })
  servicoId = serv.id
...
describe('6 – editarServico(): usuário não autorizado', () => {
  it('deve lançar NotAuthorizedError', async () => {
    ...
    const s2 = await prisma.servico.create({
      data:{ nome:'Penteado', descricao:'p', preco:40, barbeariaId: outraBarbeariaId }
    })
    await expect(
      servicoService.editarServico(
...
describe('10 – deletarServico(): sucesso', () => {
  it('deve remover serviço', async () => {
    const s3 = await prisma.servico.create({
      data:{ nome:'Temp', descricao:'t', preco:15, barbeariaId: outraBarbeariaId }
    })
