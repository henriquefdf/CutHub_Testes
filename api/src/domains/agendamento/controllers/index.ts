...
router.get(
  "/listarDoCliente/:finalizado",
  verifyJWT,
  checkTipo(usuarioTipo.CLIENTE),
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const agendamentos = await agendamentoService.listarAgendamentosCliente(
        req.Usuario!.id,
        req.params.finalizado,
      );
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const agendamentos = await agendamentoService.listarAgendamentosBarbearia(
        req.Usuario!.id,
      );
      res.status(codigoStatus.SUCESSO).json(agendamentos);
    } catch (error) {
...
router.delete(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      await agendamentoService.deletarAgendamento(
        +req.params.id,
        req.Usuario!.id,
      );
