...
describe("AgendamentoService", () => {
  ...
  describe("criarAgendamento", () => {
    it("deve criar um novo agendamento com sucesso", async () => {
      const body = { data: new Date("2025-06-10T10:00:00Z"), servicoId: 1 } as AgendamentoInterface;
      ...
      jest.mocked(prisma.barbearia.findFirst).mockResolvedValue(barbearia);
      jest.mocked(prisma.agendamento.findFirst).mockResolvedValue(null);
      jest.mocked(prisma.agendamento.create).mockResolvedValue(agendamentoCriado);

      const resultado = await agendamentoService.criarAgendamento(body, 42);
      expect(prisma.servico.findFirst).toHaveBeenCalledWith({ where: { id: +body.servicoId } });
    ...
    it("deve lançar NotAuthorizedError quando serviço não encontrado", async () => {
      ...
      await expect(agendamentoService.criarAgendamento(body, 1)).rejects.toBeInstanceOf(
        NotAuthorizedError,
      );
    });
    ...
    it("deve lançar NotAuthorizedError quando já existir agendamento no mesmo horário", async () => {
      const body = { data: new Date("2025-06-11T12:00:00Z"), servicoId: 3 } as AgendamentoInterface;
      const servico = { id: 3, barbeariaId: 9 } as any;
      const barbearia = { id: 9 } as any;
      const agendamentoExistente = { id: 55, data: body.data, servicoId: 3, barbeariaId: 9 } as any;

      jest.mocked(prisma.servico.findFirst).mockResolvedValue(servico);
      jest.mocked(prisma.barbearia.findFirst).mockResolvedValue(barbearia);
      jest.mocked(prisma.agendamento.findFirst).mockResolvedValue(agendamentoExistente);
  ...
  describe("listarAgendamentosCliente", () => {
    it("deve retornar agendamentos futuros quando finalizado = '0'", async () => {
      ...
      const agendamentos = [
        { id: 10, data: new Date("2025-06-05T09:00:00Z"), servicoId: 1, barbeariaId: 1, usuarioId: 5 },
      ] as any[];

      jest.mocked(prisma.agendamento.findMany).mockResolvedValue(agendamentos);
      jest.mocked(prisma.servico.findFirst).mockResolvedValue({ id: 1, nome: "X" } as any);
      jest.mocked(prisma.barbearia.findFirst).mockResolvedValue({ id: 1, nome: "Y" } as any);

      const resultado = await agendamentoService.listarAgendamentosCliente(5, "0");
      ...
      expect(resultado).toEqual([
        { ...agendamentos[0], servico: { id: 1, nome: "X" }, barbearia: { id: 1, nome: "Y" } },
      ]);
      jest.useRealTimers();
    });
    ...
    it("deve retornar agendamentos passados quando finalizado = '1'", async () => {
      ...
      jest.useFakeTimers().setSystemTime(now);
      const agendamentos = [
        { id: 20, data: new Date("2025-05-01T10:00:00Z"), servicoId: 2, barbeariaId: 2, usuarioId: 6 },
      ] as any[];

      jest.mocked(prisma.agendamento.findMany).mockResolvedValue(agendamentos);
      jest.mocked(prisma.servico.findFirst).mockResolvedValue({ id: 2, nome: "A" } as any);
      jest.mocked(prisma.barbearia.findFirst).mockResolvedValue({ id: 2, nome: "B" } as any);

      const resultado = await agendamentoService.listarAgendamentosCliente(6, "1");
      ...
      expect(resultado).toEqual([
        { ...agendamentos[0], servico: { id: 2, nome: "A" }, barbearia: { id: 2, nome: "B" } },
      ]);
      jest.useRealTimers();
    });
  });
  ...
  describe("listarAgendamentosBarbearia", () => {
    it("deve retornar agendamentos da barbearia quando encontrada", async () => {
      ...
      const agendamentos = [
        { id: 30, data: new Date(), servicoId: 5, barbeariaId: 7, usuarioId: 9 },
      ] as any[];
      ...
      expect(prisma.barbearia.findFirst).toHaveBeenCalledWith({ where: { usuarioId: 8 } });
      expect(prisma.agendamento.findMany).toHaveBeenCalledWith({ where: { barbeariaId: 7 } });
      expect(resultado).toEqual(agendamentos);
    });

    it("deve lançar NotAuthorizedError quando barbearia não encontrada", async () => {
      jest.mocked(prisma.barbearia.findFirst).mockResolvedValue(null);
      await expect(agendamentoService.listarAgendamentosBarbearia(99)).rejects.toBeInstanceOf(
        NotAuthorizedError,
      );
    });
  });
  ...
  describe("deletarAgendamento", () => {
    it("deve deletar agendamento quando encontrado e usuário autorizado", async () => {
      const agendamento = { id: 40, data: new Date(), servicoId: 6, barbeariaId: 3, usuarioId: 10 } as any;

      jest.mocked(prisma.agendamento.findFirst).mockResolvedValue(agendamento);
      jest.mocked(prisma.agendamento.delete).mockResolvedValue(agendamento);

      const resultado = await agendamentoService.deletarAgendamento(40, 10);
      expect(prisma.agendamento.findFirst).toHaveBeenCalledWith({ where: { id: 40 } });
      expect(prisma.agendamento.delete).toHaveBeenCalledWith({ where: { id: 40 } });
      expect(resultado).toEqual(agendamento);
    });

    it("deve lançar NotAuthorizedError quando agendamento não encontrado", async () => {
      jest.mocked(prisma.agendamento.findFirst).mockResolvedValue(null);
      await expect(agendamentoService.deletarAgendamento(50, 20)).rejects.toBeInstanceOf(
        NotAuthorizedError,
      );
    });

    it("deve lançar NotAuthorizedError quando usuário não autorizado a deletar", async () => {
      const ag = { id: 60, data: new Date(), servicoId: 7, barbeariaId: 4, usuarioId: 30 } as any;
      jest.mocked(prisma.agendamento.findFirst).mockResolvedValue(ag);
      await expect(agendamentoService.deletarAgendamento(60, 99)).rejects.toBeInstanceOf(
        NotAuthorizedError,
      );
    });
  });
  ...
  describe("listarAgendamentosBarbeariaData", () => {
    it("deve retornar agendamentos filtrados pela data corretamente", async () => {
      ...
      const agendamentos = [
        { id: 70, data: new Date("2025-07-15T09:00:00Z"), servicoId: 8, barbeariaId: 11, usuarioId: 12 },
        { id: 71, data: new Date("2025-07-15T18:00:00Z"), servicoId: 9, barbeariaId: 11, usuarioId: 13 },
      ] as any[];

      jest
        .mocked(prisma.agendamento.findMany)
        .mockResolvedValue(agendamentos);

      const resultado = await agendamentoService.listarAgendamentosBarbeariaData("11", baseDate);
