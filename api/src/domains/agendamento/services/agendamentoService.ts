...
export class agendamentoService {
  async criarAgendamento(body: AgendamentoInterface, usuarioId: number) {
    ...
    if (!servico) throw new NotAuthorizedError("Serviço não encontrado.");
    ...
    if (!barbearia) throw new NotAuthorizedError("Barbearia não encontrada.");
  ...
  async listarAgendamentosCliente(usuarioId: number, finalizado: string) {
    ...
    const agendamentosComServico = await Promise.all(
      agendamentos.map(async (agendamento: Agendamento) => {
        const servico = await prisma.servico.findFirst({
          where: {
  ...
  async deletarAgendamento(agendamentoId: number, usuarioId: number) {
    ...
    if (agendamento.usuarioId !== usuarioId)
      throw new NotAuthorizedError(
        "Você não tem permissão para cancelar esse agendamento.",
      );
