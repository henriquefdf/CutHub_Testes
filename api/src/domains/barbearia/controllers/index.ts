  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const novaBarbearia = await barbeariaService.criarBarbearia(
        req.body,
        req.Usuario!.id,
        req.file ? (req.file as unknown as Express.MulterS3.File) : null,
      );
...
router.put(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const barbeariaAtualizada = await barbeariaService.editarBarbearia(
        req.body,
        req.Usuario!.id,
        req.file ? (req.file as unknown as Express.MulterS3.File) : null,
      );
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const barbearia = await barbeariaService.listarBarbeariasPorNome(
        req.params.nome,
      );
      res.status(codigoStatus.SUCESSO).json(barbearia);
    } catch (error) {
